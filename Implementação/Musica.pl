% Definindo os fatos
%artista(nome_do_artista, lista_de_álbuns)
...
%Lista todos os artistas que pertencem a um gênero musical
...
%Verifica se um álbum é do gênero passado
eh_do_genero(Album, Genero):-
    album(Album, Generos, _),
    member(Genero, Generos).

%Lista todos os albuns de determinado gênero
albuns_do_genero(Genero, Albuns):-
    setof(Album, eh_do_genero(Album, Genero), Albuns),
    length(Albuns, N),
    format("Total de álbuns desse gênero: ~w~n", [N]).

%Informa a quantidade de músicas em um determinado álbum
qtd_musicas_por_album(Album, Quantidade) :-
    album(Album, _, Quantidade).

%Retorna qual artista detêm determinado álbum e vice-versa
